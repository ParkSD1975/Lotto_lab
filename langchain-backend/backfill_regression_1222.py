...
def backfill_1222():
    """1222회 회귀 분석 재적재 (step 2~200)"""
    print("=" * 80)
    print("1222회 weekly_regression_analysis 전체 재적재 (step 2~200)")
    print("=" * 80)
    ...
    for w in range(2, 201):
        try:
            reg_result = ensemble.predict_regression(history_1222, step=w)
            ...
            supabase.table("weekly_regression_analysis") \
                .delete().eq("target_round", 1222).eq("step", w).execute()
