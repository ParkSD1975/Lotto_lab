"""Phase 1-10: 배수 6개 카테고리 — 11 base 통합 강화."""
import numpy as np
...
# 배수 pool 정의
MULTIPLE_SETS = {
    "mult_3": {n for n in range(3,46,3)},  # 3,6,9,...,45
    "mult_4": {n for n in range(4,46,4)},  # 4,8,12,...,44
    "mult_5": {n for n in range(5,46,5)},  # 5,10,15,...,45
    "mult_7": {n for n in range(7,46,7)},  # 7,14,21,...,42
    "mult_8": {n for n in range(8,46,8)},  # 8,16,24,...,40
}
# 배수외: 위 배수 집합 합친 것의 여집합
ALL_MULT = set().union(*MULTIPLE_SETS.values())
MULTIPLE_SETS["non_mult"] = {n for n in range(1,46)} - ALL_MULT
...
class MultiplePredictor:
    ...
    카테고리 6개:
    - mult_3, mult_4, mult_5, mult_7, mult_8, non_mult
    """
    def __init__(self):
        self.markov_heads=[_Markov7() for _ in range(6)]
        self.xgb_models=None
        self._trained=False
        self.labels=["mult_3","mult_4","mult_5","mult_7","mult_8","non_mult"]
    ...
    def _extract(self,draws):
        ...
        for d in chron:
            nums=d.get("numbers",[])
            c=[sum(1 for n in nums if n in MULTIPLE_SETS[lab]) for lab in self.labels]
            counts.append(c)
        ...
        if XGB_AVAILABLE:
            ...
            for cat_idx in range(6):
                ...
                try:
                    ...
                    print(f"[Multiple] XGB cat {cat_idx} train fail: {e}")
                    self.xgb_models.append(None)
    ...
    def predict(self,draws,target_round):
        ...
        for i in range(6):
            ...
            lab=self.labels[i]; M=POOL_SIZES[lab]
        ...
        return {"indicator_name":"multiple_distribution","per_category":per_cat}

    def _baseline(self):
        u=np.full(7,1/7)
        return {"indicator_name":"multiple_distribution","per_category":{lab:{"absolute_dist":u.tolist(),"expected_count":3,"current_pool_size":POOL_SIZES[lab],"expected_ratio":3/POOL_SIZES[lab],"top_class":0,"top_class_prob":1/7,"narrative":"not trained","model_contributions":{}} for lab in self.labels}}
...
def validate_walk_forward(n_rounds=50):
    ...
    p=MultiplePredictor(); r=BaselineRunner()
    cat_res={}
    ...
    return {"n_categories":6,"avg_predictor_ce":float(apc),"avg_frequency_ce":float(afc),"improvement_pct":(afc-apc)/max(afc,1e-9)*100,"wins_frequency":apc<afc,"per_category":cat_res}
...
def main():
    ...
    if args.smoke:
        ...
        print("[Multiple] smoke PASS")

    if args.validate>0:
        print(f"[Multiple] validating {args.validate} rounds...")
