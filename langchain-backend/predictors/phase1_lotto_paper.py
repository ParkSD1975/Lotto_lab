"""Phase 1-9: 로또용지 14개 카테고리 (7행 + 7열) — 11 base 통합 강화."""
import numpy as np
...
def _number_to_grid(n):
    """번호 1~45를 7×7 그리드 (row, col) 변환.
...
class LottoPaperPredictor:
    ...
    카테고리 14개:
    - row_0 ~ row_6: 각 행에 본번호 6개 중 몇 개 출현 (0~6)
    - col_0 ~ col_6: 각 열에 본번호 6개 중 몇 개 출현 (0~6)
    """
    def __init__(self):
        self.markov_heads=[_Markov7() for _ in range(14)]
    ...
    def _extract(self,draws):
        ...
        for d in chron:
            ...
            col_counts=[0]*7
            for n in nums:
                r,c=_number_to_grid(n)
                if r is not None:
                    row_counts[r]+=1
                if c is not None:
                    col_counts[c]+=1
            counts.append(row_counts+col_counts)
        ...
        if XGB_AVAILABLE:
            ...
            for cat_idx in range(14):
                ...
                try:
                    ...
                    print(f"[LottoPaper] XGB cat {cat_idx} train fail: {e}")
                    self.xgb_models.append(None)
    ...
    def predict(self,draws,target_round):
        ...
        per_cat={}
        labels=[f"row_{i}" for i in range(7)]+[f"col_{i}" for i in range(7)]
        # pool_size: 각 행/열의 번호 개수
        # row: 0~5 = 7개, row 6 = 3개 (43,44,45)
        # col: 모든 열 = 6~7개
        pool_sizes=[7,7,7,7,7,7,3,7,7,7,7,7,7,7]
        ...
        return {"indicator_name":"lotto_paper_distribution","per_category":per_cat}

    def _baseline(self):
        u=np.full(7,1/7)
        labels=[f"row_{i}" for i in range(7)]+[f"col_{i}" for i in range(7)]
        pool_sizes=[7,7,7,7,7,7,3,7,7,7,7,7,7,7]
        return {"indicator_name":"lotto_paper_distribution","per_category":{labels[i]:{"absolute_dist":u.tolist(),"expected_count":3,"current_pool_size":pool_sizes[i],"expected_ratio":3/pool_sizes[i],"top_class":0,"top_class_prob":1/7,"narrative":"not trained","model_contributions":{}} for i in range(14)}}
...
def validate_walk_forward(n_rounds=50):
    ...
    p=LottoPaperPredictor(); r=BaselineRunner()
    cat_res={}
    labels=[f"row_{i}" for i in range(7)]+[f"col_{i}" for i in range(7)]
    ...
    return {"n_categories":14,"avg_predictor_ce":float(apc),"avg_frequency_ce":float(afc),"improvement_pct":(afc-apc)/max(afc,1e-9)*100,"wins_frequency":apc<afc,"per_category":cat_res}
...
def main():
    ...
    if args.smoke:
        ...
        print("[LottoPaper] smoke PASS")

    if args.validate>0:
        print(f"[LottoPaper] validating {args.validate} rounds...")
