"""Phase 1-12: 미출현그룹 4개 카테고리 — 11 base 통합 강화."""
import numpy as np
...
class MissingGroupPredictor:
    ...
    카테고리 4개:
    - near_gap (gap<5), mid_gap (5~14), far_gap (15~29), very_far_gap (>=30)
    gap = 각 번호의 직전 출현 이후 경과 회차 수
    """
    def __init__(self):
        self.markov_heads=[_Markov7() for _ in range(4)]
        self.xgb_models=None
        self._trained=False
        self.labels=["near_gap","mid_gap","far_gap","very_far_gap"]

    def _compute_gaps(self, draws):
        """각 번호(1~45)의 gap 계산.

        returns: dict {number: gap}
        """
        if len(draws)==0:
            return {n: 999 for n in range(1,46)}

        last_occurrence = {n: -1 for n in range(1,46)}
        chron = list(reversed(draws))  # 오래된 순
        for i, d in enumerate(chron):
            for num in d.get("numbers",[]):
                if last_occurrence[num] == -1:
                    last_occurrence[num] = i

        current_round = len(chron) - 1
        gaps = {}
        for n in range(1,46):
            if last_occurrence[n] == -1:
                gaps[n] = 999  # 미출현
            else:
                gaps[n] = current_round - last_occurrence[n]
        return gaps
    ...
    def _extract(self,draws):
        ...
        for i in range(len(chron)):
            # i 시점에서 각 번호의 gap 계산 (0~i 회차 기준)
            hist_draws = chron[:i+1]
            gaps = self._compute_gaps(hist_draws)
            # 현재 회차 본번호
            ...
            counts.append([near, mid, far, very_far])
        ...
        if XGB_AVAILABLE:
            ...
            for cat_idx in range(4):
                ...
                try:
                    ...
                    print(f"[MissingGroup] XGB cat {cat_idx} train fail: {e}")
                    self.xgb_models.append(None)
    ...
    def predict(self,draws,target_round):
        ...
        per_cat={}
        pool_sizes=[45,45,45,45]  # 동적이지만 단순화: 전체 번호 pool
        ...
        return {"indicator_name":"missing_group_distribution","per_category":per_cat}

    def _baseline(self):
        u=np.full(7,1/7)
        return {"indicator_name":"missing_group_distribution","per_category":{lab:{"absolute_dist":u.tolist(),"expected_count":3,"current_pool_size":45,"expected_ratio":3/45,"top_class":0,"top_class_prob":1/7,"narrative":"not trained","model_contributions":{}} for lab in self.labels}}
...
def validate_walk_forward(n_rounds=50):
    ...
    p=MissingGroupPredictor(); r=BaselineRunner()
    cat_res={}
    ...
    return {"n_categories":4,"avg_predictor_ce":float(apc),"avg_frequency_ce":float(afc),"improvement_pct":(afc-apc)/max(afc,1e-9)*100,"wins_frequency":apc<afc,"per_category":cat_res}
...
def main():
    ...
    if args.smoke:
        ...
        for k in list(out["per_category"].keys()):
            v=out["per_category"][k]
            print(f"  {k}: top={v['top_class']} P={v['top_class_prob']:.3f}")
        print("[MissingGroup] smoke PASS")

    if args.validate>0:
        print(f"[MissingGroup] validating {args.validate} rounds...")
