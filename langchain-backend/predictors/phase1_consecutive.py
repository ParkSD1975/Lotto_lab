"""Phase 1-8: 연번 1개 카테고리 — 11 base 통합 강화."""
import numpy as np
...
class ConsecutivePredictor:
    """연번 예측기.

    카테고리 1개:
    - consecutive_count: 연속 번호 개수 (예: [3,4,5,12,25,30] → 3개 연속 1개)
    """
    def __init__(self):
    ...
    def _extract(self,draws):
        ...
        for d in chron:
            ...
            if streak>=2:
                consec+=streak
            counts.append(min(consec,6))  # 0~6 범위 제한
        ...
        if XGB_AVAILABLE:
            ...
            if X is not None:
                ...
                try:
                    ...
                    print(f"[Consecutive] XGB train fail: {e}")
                    self.xgb_model = None
    ...
    def predict(self,draws,target_round):
        ...
        M=6
        return {"indicator_name":"consecutive_distribution","per_category":{"consecutive_count":{
            ...
            "narrative":f"연번 pool {M}개 중 {int(np.argmax(ap))}개 가능성 {ap.max()*100:.1f}%",
            "model_contributions":contrib}}}

    def _baseline(self):
        u=np.full(7,1/7)
        return {"indicator_name":"consecutive_distribution","per_category":{"consecutive_count":{"absolute_dist":u.tolist(),"expected_count":3,"current_pool_size":6,"expected_ratio":3/6,"top_class":0,"top_class_prob":1/7,"narrative":"not trained","model_contributions":{}}}}
...
def validate_walk_forward(n_rounds=50):
    ...
    p=ConsecutivePredictor(); r=BaselineRunner()
    ...
    for tr in rounds:
        ...
        pd=np.array(out["per_category"]["consecutive_count"]["absolute_dist"])
...
def main():
    ...
    if args.smoke:
        ...
        v=out["per_category"]["consecutive_count"]
        print(f"  consecutive_count: top={v['top_class']} P={v['top_class_prob']:.3f}")
        print("[Consecutive] smoke PASS")

    if args.validate>0:
        print(f"[Consecutive] validating {args.validate} rounds...")
