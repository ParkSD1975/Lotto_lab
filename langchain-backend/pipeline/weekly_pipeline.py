...
본 모듈은 weekly_pipeline_v2.py로 대체될 예정. v2가 single source of truth.

마이그레이션 계획:
- main.py / routes/pipeline.py가 v1 import 중 → 별도 PR에서 v2 호출로 전환
- 그 후 본 파일 삭제 (T-1-4 결정 C: weekly_pipeline.py 삭제)

본 PR에서는 호환성 유지 위해 그대로 보존.
"""
