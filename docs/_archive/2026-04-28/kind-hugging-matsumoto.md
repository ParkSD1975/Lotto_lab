...
총합, 끝수합, AC값, 고저, 홀짝, **이월수(2변형: 정확/보너스포함)**, **연번**, **이웃수(2변형: 정확/보너스포함)**, 끝수0~9 (10개 독립), 번호대(5개 독립: 단번대/10대/20대/30대/40대), 9궁(5분할 9개 독립), 로또용지(가로7+세로7 = 14개 독립), 소수, 합성수, **제곱수**, 삼각수, 동형수, 배수(5개 독립: 3·4·5·7·8), 핫콜드(5/10/15/20기준), 미출현그룹(0-5/6-10/11-15/16+), 회귀(2~200)
...
- 학습 순서: Markov → XGBoost → CNN → LSTM → Transformer
...
**Part B — 메인 1~45 모델의 sum feature 강화**
...
8. `langchain-backend/models/lstm_model.py` — INPUT_DIM 65 → 80, feature 빌더 호출
9. `langchain-backend/models/transformer_model.py` — INPUT_DIM 57 → 72
...
**Part E — 메인 1~45 모델에 endings feature 주입**
...
7. `langchain-backend/models/lstm_model.py` — INPUT_DIM 80 → ~95
8. `langchain-backend/models/transformer_model.py` — INPUT_DIM 72 → ~85
9. `langchain-backend/models/xgboost_model.py` — feature 약 13개 추가
...
6. `langchain-backend/models/lstm_model.py` — INPUT_DIM ~95 → ~100
7. `langchain-backend/models/transformer_model.py` — INPUT_DIM ~85 → ~90
8. `langchain-backend/models/xgboost_model.py` — feature 5개 추가
...
7. `models/lstm/transformer/xgboost_model.py` — 7D 분포 + 통계 추가
...
4. `models/lstm/transformer/xgboost_model.py` — 7D 분포 + previous_round 변수 추가
...
7. `models/lstm/transformer/xgboost_model.py` — 6D + 4 컨텍스트 변수
...
5. `models/lstm/transformer/xgboost_model.py` — 7D + 2 컨텍스트
...
5. `models/lstm/transformer/xgboost_model.py` — 끝수별 expected 10 + dist 70(10×7) 추가
...
4. `models/lstm/transformer/xgboost_model.py` — 5×7 분포 + 5 expected + 80 임베딩 + 10 pairwise
...
4. `models/lstm/transformer/xgboost_model.py` — 9×7 분포 63 + 9 expected + 144 임베딩 + 8 인접쌍
...
4. `models/lstm/transformer/xgboost_model.py` — 14×7 + 14 expected + 224 임베딩 + 24 pairwise + cnn_grid_logits[49]
...
3. `models/lstm/transformer/xgboost_model.py` — 5×7 + 5 expected + 80 임베딩 + 10 pairwise + 매핑 매트릭스
...
4. `models/lstm/transformer/xgboost_model.py` — 3×7 + 3 expected + 48 임베딩 + 3 pairwise + 비율 변수 5개
...
3. `models/lstm/transformer/xgboost_model.py` — 7 분포 + 1 expected + 5 컨텍스트
...
3. `models/lstm/transformer/xgboost_model.py` — 각 7 분포 + 1 expected + 5 컨텍스트
...
3. `models/lstm/transformer/xgboost_model.py` — 4×7 + 4 expected + 64 임베딩 + 6 pairwise + 4 current_group_size
...
3. `models/lstm/transformer/xgboost_model.py` — 12×7 + 12 expected + 192 임베딩 + 30 pairwise + 12 pool_size
...
- 적용: 모든 1~45 binary 분류 모델 (LSTM, CNN, GNN, Transformer)
...
- `langchain-backend/models/independent_count_predictor.py` (강화 패턴, 끝수/번호대/9궁/로또용지/배수/소수/삼각수/제곱수/동형수/미출현/핫콜드)
- `langchain-backend/validation/timeseries_cv.py`, `validation/baselines.py`

**수정 핵심**:
- `services/filter_stats.py` (대폭 — 모든 필터 메서드 강화 + 신규 메서드 6개+)
- `pipeline/weekly_pipeline_v2.py::_run_analysis()` (Phase 1~4 호출 추가)
- `models/lstm/transformer/cnn/xgboost/gnn_model.py` (Focal Loss 수정, 정규화, feature 확장)
- `models/ensemble.py::train_all` (walk-forward CV, baseline)
- `config.py` (모든 새 카테고리 정의, RANDOM_SEED, 하이퍼파라미터)
